    ...
    public void Update()
    {
        ...
        if (!(orbState == OrbState.SPAWNING))
        {
            ...
            foreach (Collider2D activeCollider in allColliders)
            {
                ...
                {
                    if (activeCollider.transform.parent.GetComponent<PlayerActions>().getCurrentHealth() != activeCollider.transform.parent.GetComponent<PlayerActions>().getMaxHealth())
                    {
                        found = true;
